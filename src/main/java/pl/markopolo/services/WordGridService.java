...
public class WordGridService {
            ...
            for (String word: words) {
                Collections.shuffle(coordinates);
                    for (Coordinate coordinate : coordinates) {
                        int x = coordinate.x;
                        int y = coordinate.y;
                        Direction selectedDirection = getFittingDirection(contents, word, coordinate);
        ...
        private void randomFillGrid(char[][] contents) {
            int gridSize = contents[0].length;
            String allCapLetters = "ABCDEFGHIJKLMNOPRSTUWXYZ";
            for (int i = 0; i < gridSize; i++) {
                for (int j = 0; j < gridSize; j++) {
                    if (contents[i][j] == '_') {
        ...
        private Direction getFittingDirection(char[][] contents, String word, Coordinate coordinate) {
            ...
            switch (direction) {
                ...
                case DIAGONAL -> {
                    if (coordinate.y + wordLength > gridSize ||
                            coordinate.x + wordLength > gridSize) return false;
                    for (int i = 0; i < wordLength; i++) {
                ...
                case DIAGONAL_INVERSE -> {
                    if (coordinate.y < wordLength ||
                            coordinate.x < wordLength) return false;
                    for (int i = 0; i < wordLength; i++) {
                        if (contents[coordinate.x - i][coordinate.y - i] != '_') return false;
                    }
                }
